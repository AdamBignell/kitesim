...
import PlayerCapabilitiesProfile from './generation/PlayerCapabilitiesProfile'; // Import PCP
...
export default class GameScene extends Phaser.Scene {
  ...
  preload() {
    ...
    // Generate a simple tile texture for our map
    const graphics = this.make.graphics({ x: 0, y: 0, add: false });
    graphics.fillStyle(0x222222); // A dark grey
  ...
  create() {
    ...
    // Create the PlayerCapabilitiesProfile instance
  ...
  togglePlayerControl(isUICall = false) {
    if (!isUICall && this.isAIControlled) {
  ...
  update() {
    if (this.player.y > this.game.config.height + 50) {
      this.player.setPosition(100, 450); // Reset position
  ...
  redrawLevel(gameSize) {
    ...
    // If a tilemap layer already exists, destroy it
    if (this.mapLayer) {
    ...
    const map = this.make.tilemap({ data: gridArray, tileWidth: TILE_SIZE, tileHeight: TILE_SIZE });
    const tileset = map.addTilesetImage('tile');
    this.mapLayer = map.createLayer(0, tileset, 0, 0);

    map.setCollision(1); // Tile with index 1 is solid
