...
describe('GreedyMesher', () => {
  ...
  it('should mesh a single tile', () => {
    ...
    grid.setTile(5, 5, 1);
    const meshes = GreedyMesher.mesh(grid);
    expect(meshes).toEqual([{ x: 5, y: 5, width: 1, height: 1 }]);
  });
  ...
  it('should mesh a horizontal line', () => {
    ...
    grid.setTile(5, 5, 1);
    const meshes = GreedyMesher.mesh(grid);
    expect(meshes).toEqual([{ x: 3, y: 5, width: 3, height: 1 }]);
  });
  ...
  it('should mesh a vertical line', () => {
    ...
    grid.setTile(5, 5, 1);
    const meshes = GreedyMesher.mesh(grid);
    expect(meshes).toEqual([{ x: 5, y: 3, width: 1, height: 3 }]);
  });
  ...
  it('should mesh a solid rectangle', () => {
    ...
    const meshes = GreedyMesher.mesh(grid);
    expect(meshes).toEqual([{ x: 3, y: 3, width: 3, height: 3 }]);
  });
